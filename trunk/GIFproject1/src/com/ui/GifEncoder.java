...
//				color.
	...
	public void encode(Bitmap bufferedimage, DataOutputStream dataoutputstream, Hashtable hashtable) throws Exception {
		try {
...
//			 e = bufferedimage.getRGB(0, 0, a, g, null, 0, a);
			...
			/*
			 * else if(b.equals("optimized")) { try { int k4 = Integer.parseInt(hashtable.get("colors").toString()); for(h = 1; k4
			 * - 1 >> h > 0; h++) { } i = new int[1 << h]; CSelectiveQuant cselectivequant = new CSelectiveQuant(); for(int j5 =
			 * 0; j5 < e.length; j5++) { cselectivequant.addPixel(e[j5]); }
			 * 
			 * boolean flag = f > 0; int k5 = flag ? 1 : 0; int ai1[] = cselectivequant.createPalette(k4 - k5); for(int l5 = 0; l5
			 * < i.length; l5++) { try { i[l5] = ai1[l5 - k5]; } catch(ArrayIndexOutOfBoundsException
			 * arrayindexoutofboundsexception) { i[l5] = 0; } }
			 * 
			 * if(flag) { i4 = 0; for(int i6 = 0; i6 < f; i6++) { cselectivequant.setIndex(c[i6], -1); }
			 * 
			 * } for(int j6 = 0; j6 < e.length; j6++) { e[j6] = cselectivequant.getIndex(e[j6]) + k5; }
			 * 
			 * } catch(NumberFormatException numberformatexception) { CmsLogger.logInfo("Parameter: 'colors' is malformated...");
			 * return; } }
			 */
